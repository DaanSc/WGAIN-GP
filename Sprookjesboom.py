...
out_path = "C:/Users/Lars Hurkmans/Downloads/Master vakken/Case study/Data/GAINtje hier GAINtje daar/Gouden kaartje/Bol_op_Hol/"
...
empirical_path =  "C:/Users/Lars Hurkmans/Downloads/Master vakken/Case study/Data/GAINtje hier GAINtje daar/Gouden kaartje/Empirical/"
...
# Own files
from EmbedPret import RestIndex, EmbeddedOrder
from utils import normalization, renormalization
from SupportGAIN import Impute
...
def TreeModel(tree_data, currentday=0,seed=123):
    ...
    for order in tqdm(range(obs)):
        ...
        if(predicted_Shipmentdays>=0):
            predicted_transportername = tree_data["transporterName"].iloc[order]
        ...
        while(outNode>=0):
           ...
           if(np.sum(inNode==skip_node_indices)!=1):
               ...
               if(len(Baseline[inNode])>1):
                ...
                if(inNode==2):
                    ship_element= np.array(np.where(np.random.multinomial(1,(Baseline[inNode][1]))==1))[0][0]
...
def TreeStats(tree_data, predict_nodes, end_nodes, 
    ...
    pred_end=np.transpose(np.vstack((predict_nodes,end_nodes)))
    ...
    if (talk==True):
        ...
        print("Precision unhappy: ", precision_happy)
...
def dayResults (GAIN_type, last_day):
    ...
    for day in tqdm(range(len(days))):
        if(GAIN_type!= "Baseline"):
            Model = GAIN_Model(tree_data, GAIN_type=GAIN_type, currentday=day,seed=seed)   
        if(GAIN_type=="Baseline"):
...
if(answer==0):
    ...
    print("That is a pitty, no picasso for yo")
