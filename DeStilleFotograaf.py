...
def PlotHists(compare, Impute_Cols, plotcols=1, out_path=None,test=False):
    ...
    if(len(Impute_Cols)==2):
        ...
        for idx,ax in enumerate(a):
            ax.hist(compare[idx][0], bins=len(np.unique(compare[idx][0,])), ec="k", density=True, stacked=True, label="Actual")
            ax.hist(compare[idx][1], bins=len(np.unique(compare[idx][1,])), ec="k", density=True, stacked=True,
    ...
    if(len(Impute_Cols)==1):
        ...
        f.text(-0.003, 0.5, 'Density', va='center', rotation='vertical', size=label_size)
        a.hist(compare[0][0], bins=len(np.unique(compare[0][0,])), ec="k", density=True, stacked=True,
               label="Actual")
        a.hist(compare[0][1], bins=len(np.unique(compare[0][1,])), ec="k", density=True, stacked=True,
               alpha=0.6, label="Imputed")
