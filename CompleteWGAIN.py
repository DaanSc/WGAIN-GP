...
# WGAIN Function. We are trying to impute data from some tabular data form.
# The steps of the GAIN are as follows:
    ...
    # 3. Define Critic: The critic is used to impute real from imputed data. The critic assumes that the embedded variables
    #    are embedded, and therefore expects a higher dimension of the data-matrix than the original data.  
    # 4. Losses & Optimizers: Define Loss functions for generator and critic as well as the used optimizers. 
    # 5. Training process: Define train process for generator and critic, as well as an overal training scheme. This is based on
...
def WGAIN(actual_data, data_x,gain_parameters, ceiling,
    ...
    def Critic():
        ...
        # Layer 3
        model.add(layers.Dense(dim, use_bias=False, activation='tanh'))
    ...
    def Opt_Critic():
        return(tf.keras.optimizers.RMSprop(learning_rate=Learning_Rate, clipvalue=Weight_Clip))
    ...
    # For the critic, we need to have a loss function that is able to 
    ...
    def Train_Generator(real_data, Generator_Input, H_Batch):
        ...
        with tf.GradientTape() as Backprop_Generator:
            # Create generator output used for critic
            ...
            if (conditioned_critic == True):
                ...
                # Convert data such that it is readable for critic
                Disc_Input=  tf.convert_to_tensor(np.hstack((Generated_Impute, H_impute)))
            # Critic value of generated data
    ...
    def Train_Critic(fake, real, H):
        ...
        with tf.GradientTape() as Backprop_Critic:
          ...
          # Critic gets the imputed data from generator as input + hint matrix
          ...
          Critic_Loss = Loss_Critic(D_Prob_F, D_Prob_R)
          # Compute gradient
    ...
    def train(real_data, Input_Data, M, H, Batches):
        ...
        for epoch in tqdm(range(Epochs)):
            ...
            #  Compute losses for critic, generator and MSE (generator)
            ...
            if((Plot==True)): 
                ...
                if((epoch+1)%Plot_Epoch==0):
                    ...
                    # Plot all images
                    Complete=Progress_Plot(2,2,Combined,start,epoch, Var_Names, name="Embed WGAIN")
                    plt.show()
    ...
    # Function that computes both critic and generator losses used for plots
    ...
    def All_Lost(Disc_Loss, MSE_Loss, Gen_Loss, Generator_Input, H, M_Big):
        ...
        # Fake and real data for critic loss
        ...
        # Compute Loss critic
        Critic_Loss= Loss_Critic(F_Prob,R_Prob)
    ...
    # Create Hint matrix, used for critic
